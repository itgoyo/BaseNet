...
/**
 * name: Talk915_Android_H5
 * package: com.zhon.baselib.utils
 * author: itgoyo
 * github: https://github.com/itgoyo
 * time: 2018/11/20 09:26.
 * desc:
 * email: itgoyo@gmail.com
 */

public final class DiyLoggingInterceptor implements Interceptor {
...
//                Platform.get().log(message);
    ...
    public DiyLoggingInterceptor() {
        this(Logger.DEFAULT);
    }

    public DiyLoggingInterceptor(Logger logger) {
        this.logger = logger;
    }
    ...
    public DiyLoggingInterceptor setLevel(Level level) {
...
//                    L.sendLogToServer("httpDetail",sb.toString());
    ...
    public void printLogToService(Chain chain, int code) throws IOException {
        ...
        if (!logBody || !hasRequestBody) {
            Log.i("djx","--> END " + request.method());
        } else if (bodyEncoded(request.headers())) {
            Log.i("djx","--> END " + request.method() + " (encoded body omitted)");
        }else{
            Buffer buffer = new Buffer();
            requestBody.writeTo(buffer);
...
//        L.sendLogToServer("httpDetail",sb.toString());
        Log.i("djx", "printLogToService: "+sb.toString());
